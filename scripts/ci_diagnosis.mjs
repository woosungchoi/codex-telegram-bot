...
const CLASSIFIERS = [
  ...
  },
  {
    ...
    confidence: "high",
    patterns: [
      /npm (?:ERR!|error) code (?:EUSAGE|ELOCKVERIFY)/i,
      /npm ci can only install packages/i,
      /package\.json and package-lock\.json are not in sync/i,
      /Missing: .* from lock file/i,
    ],
    recommendedCommands: [
      "npm ci",
      "package.json/package-lock.json 동기화 여부를 확인하세요.",
    ],
  },
  {
    type: "lint-failure",
    ...
    confidence: "medium",
    patterns: [
      /eslint.*(?:error|failed)/i,
      /(?:error|failed).*eslint/i,
      /\b[1-9]\d* problems? \([1-9]\d* errors?/i,
    ],
  ...
  },
  {
    ...
    recommendedCommands: [
      "npm test",
      "필요하면 특정 테스트 파일만 node --test로 재실행하세요.",
    ],
  },
  {
    type: "format-failure",
    ...
    patterns: [
      ...
      /prettier.*(?:error|failed)/i,
      /(?:error|failed).*prettier/i,
    ],
    recommendedCommands: [
  ...
  },
  {
    ...
    patterns: [
      /npm audit report/i,
      /\b[1-9]\d* vulnerabilities?\b/i,
      /severity:\s*(?:moderate|high|critical)/i,
    ],
    recommendedCommands: [
      "npm audit --audit-level=moderate",
      "취약 dependency 업데이트 PR을 확인하세요.",
    ],
  },
  {
    ...
    patterns: [
      ...
      /npm (?:warn|error|ERR!) audit (?:429|5\d\d)/i,
      /Invalid package tree/i,
      /\b(?:429|502|503|504) (?:Too Many Requests|Bad Gateway|Service Unavailable|Gateway Timeout)\b/i,
      /npm ERR! network/i,
      /getaddrinfo/i,
    ],
  ...
  },
  {
    ...
    recommendedCommands: [
      "workflow permissions 블록과 GITHUB_TOKEN 권한을 확인하세요.",
    ],
  },
  {
    ...
    confidence: "medium",
    patterns: [
      /CODEX_ACCESS_TOKEN/i,
      /Codex OAuth login failed/i,
      /Codex CLI package could not be installed/i,
    ],
    recommendedCommands: [
      "Codex AI 진단이 필요할 때만 CODEX_ACCESS_TOKEN secret을 갱신하세요.",
    ],
  },
];
